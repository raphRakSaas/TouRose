...
function normalizeKeywords(keywords: { fr?: string[] } | string[] | undefined): string[] {
  ...
  if (location?.name || location?.uid) {
    ...
    const placeLinks = (location.links ?? [])
      .map((link) => ({
        ...
        url:
          typeof link.url === 'string' ? link.url : typeof link.link === 'string' ? link.link : '',
      }))
    ...
    place = {
      ...
      website_url: isNonEmptyUrl(location.website) ? location.website?.trim() : undefined,
