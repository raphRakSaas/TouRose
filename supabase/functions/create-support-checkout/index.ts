...
Deno.serve(async (request) => {
  ...
  if (!stripeSecretKey) {
    return jsonResponse({ error: "STRIPE_SECRET_KEY is not configured", code: "misconfigured" }, 500);
  }
  ...
  formBody.set("line_items[0][price_data][product_data][name]", `Soutien TouRose — ${productLabel}`);
