...
Deno.test("normalizeOpenAgendaEvent maps fixture shape", async () => {
  const eventRow: OpenAgendaEvent = {
    ...
    location: {
      ...
      latitude: 43.6,
      longitude: 1.44,
      description: { fr: "Lieu emblématique de Toulouse." },
      access: { fr: "Métro A, station Capitole." },
      ...
      image: {
        ...
        filename: "place.base.jpg",
        variants: [{ type: "full", filename: "place.full.jpg" }],
      },
      imageCredits: "Photographe du lieu",
    },
    timings: [{
      begin: "2026-08-01T20:00:00+02:00",
      end: "2026-08-01T22:00:00+02:00",
    }],
    image: {
      base: "https://cdn.openagenda.com/main/",
  ...
  if (!normalized.place || normalized.place.external_id !== "7") {
    throw new Error("place");
  }
  if (normalized.place.description !== "Lieu emblématique de Toulouse.") {
    throw new Error("place description");
  }
  if (normalized.place.details.access !== "Métro A, station Capitole.") {
    throw new Error("place access");
  }
  if (normalized.place.details.links.length !== 1) {
    throw new Error("place links");
  }
  if (
    normalized.place.image?.remote_url !==
      "https://cdn.openagenda.com/main/place.full.jpg"
  ) {
    throw new Error("place image");
  }
  if (normalized.occurrences.length !== 1) throw new Error("occurrences");
  if (!normalized.payload_hash) throw new Error("hash");
  if (
    normalized.image?.remote_url !==
      "https://cdn.openagenda.com/main/event.full.jpg"
  ) {
    throw new Error("image url");
  }
  if (
    normalized.image.attribution_text !==
      "Photo : Photographe test · OpenAgenda"
  ) {
...
Deno.test("buildOpenAgendaEventDetails tolerates missing fields", () => {
  ...
  if (details.accessibility.length !== 0) {
    throw new Error("accessibility default");
  }
  if (details.keywords.length !== 0) throw new Error("keywords default");
  if (details.registration.length !== 0) {
    throw new Error("registration default");
  }
