...
Deno.serve(async (request) => {
  ...
  if (!lastRun) {
    ...
    const hoursSinceSuccess = (Date.now() - new Date(finishedAt).getTime()) / (1000 * 60 * 60);
    if (hoursSinceSuccess > staleAfterHours) {
      alerts.push({
        alert_type: 'import_stale',
        severity: 'warning',
        message: `Dernier import réussi il y a ${hoursSinceSuccess.toFixed(1)} h (seuil ${staleAfterHours} h).`,
      });
    }
  }
